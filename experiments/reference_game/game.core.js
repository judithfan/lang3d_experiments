...
var game_core = function(options){
  ...
  this.iterationName = 'chairs2k_expansion_only';
...
game_core.prototype.newRound = function() {
  ...
  if(this.roundNum == this.numRounds - 1) {
    ...
    this.roundNum += 1;
    sendStim(this); // this is defined in app.js
  }
};

function sendStim(game) {
  sendPostRequest('http://localhost:4200/db/getstims', {
    json: {
      dbname: 'stimuli',
      colname: 'chairs2k_expansion_only',
      numTrials: 1,
      gameid: game.gameID
    }
  }, (error, res, body) => {
	  ...
	  game.trialInfo = {
	  roles: _.zipObject(_.map(game.players, p =>p.id),
			     _.reverse(_.values(game.trialInfo.roles)))
	};

        game.stim = body[0];
	game.objects = game.makeTrial()
...
game_core.prototype.makeTrial = function () {
    var stim = this.stim;
